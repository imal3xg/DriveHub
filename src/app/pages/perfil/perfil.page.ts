...
export class PerfilPage implements OnInit {
  ...
  constructor(
    ...
    private media:MediaService,
    private modal:ModalController,
    private anuncioForm:AnuncioFormComponent
  ) {}
  ...
  onNewAnuncio() {
    // Abre un modal con el componente del formulario para crear un nuevo anuncio
    this.presentForm(null, async(result) => {
      if (result && result.data) {
        // Puedes acceder a los datos del formulario a través del componente del formulario
        const formData = await this.anuncioForm.getFormData();
        // Puedes realizar alguna lógica adicional aquí antes de guardar el nuevo anuncio
  
        // Llama a un servicio para crear el nuevo anuncio
        this.misanuns.addAnuncio(formData).subscribe({
          next: (response) => {
            // Muestra un mensaje de éxito o realiza acciones adicionales
            this.showToast('Anuncio creado exitosamente');
          },
